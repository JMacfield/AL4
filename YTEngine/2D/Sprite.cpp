...
void Sprite::CreateVartexData(const Vector4& a, const Vector4& b) {
	vertexResource = directXCommon_->CreateBufferResource(directXCommon_->GetDevice().Get(), sizeof(VertexData) * 6);
...
void Sprite::CreateTransform() {
	wvpResource_ = DirectXCommon::CreateBufferResource(directXCommon_->GetDevice().Get(), sizeof(Matrix4x4));
