...
void SrvDescriptorHeap::AddPtr(uint32_t index, D3D12_DESCRIPTOR_HEAP_TYPE type) {
	SrvHandleCPU_[index].ptr += directXCommon_->GetDevice()->GetDescriptorHandleIncrementSize(type);
	SrvHandleGPU_[index].ptr += directXCommon_->GetDevice()->GetDescriptorHandleIncrementSize(type);
}
